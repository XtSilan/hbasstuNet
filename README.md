...
- 登录页可分别启用自动登录；设置页可控制当前 Windows 用户的开机自启动
...
程序为单文件便携版。若移动或重命名 exe，先从新位置手动启动一次；程序会检测并同步 Windows 启动项到当前路径，之后开机启动将继续有效。任务管理器禁用启动项时，程序不会自动重新启用；请在设置页重新打开“开机自启动”才会恢复。
...
2. 选择已发现的学生或教师网络。
3. 输入校园网账号、密码与运营商。
4. 在登录页勾选“保存密码”和“自动登录”后，后续启动会自动发现校园 Wi-Fi 并完成认证；如需登录 Windows 后在托盘后台运行，再在设置页启用“开机自启动”。
